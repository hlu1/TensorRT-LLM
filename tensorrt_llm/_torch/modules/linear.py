...
def load_weights_vanilla_helper(module: torch.nn.Module, weights: List[Dict],
                                device: torch.device):
...
def load_weights_fused_qkv_helper(
        module: torch.nn.Module, weights: List[Dict], device: torch.device
...
def load_weights_fused_gate_up_helper(
        module: torch.nn.Module, weights: List[Dict],
...
class LinearMethodBase(QuantizeMethodBase):
    ...
    @abstractmethod
    def create_weights(self, module: torch.nn.Module, in_features: int,
                       out_features: int, bias: bool, dtype: torch.dtype, *args,
                       **kwargs):
        raise NotImplementedError

    @abstractmethod
    def apply(self, module: torch.nn.Module, input: torch.Tensor,
    ...
    def load_weights(self, module: torch.nn.Module, weights: List[Dict],
        ...
        if weight_mode == WeightMode.VANILLA:
            self.load_weights_vanilla(module, weights, device)
        elif weight_mode == WeightMode.FUSED_QKV_LINEAR:
            self.load_weights_fused_qkv_linear(module, weights, device)
        elif weight_mode == WeightMode.FUSED_GATE_UP_LINEAR:
            self.load_weights_fused_gate_up_linear(module, weights, device)
        else:
            raise ValueError(f'unsupported weight mode: {weight_mode}')

    @abstractmethod
    def load_weights_vanilla(self, module: torch.nn.Module, weights: List[Dict],
                             device: torch.device):
        """
        Load weights for the VANILLA linear method.
        """
        raise NotImplementedError

    @abstractmethod
    def load_weights_fused_qkv_linear(self, module: torch.nn.Module,
                                      weights: List[Dict],
                                      device: torch.device):
        """
        Load weights for the FUSED_QKV_LINEAR linear method.
        """
        raise NotImplementedError

    @abstractmethod
    def load_weights_fused_gate_up_linear(self, module: torch.nn.Module,
                                          weights: List[Dict],
                                          device: torch.device):
        """
        Load weights for the FUSED_GATE_UP_LINEAR linear method.
        """
        raise NotImplementedError
...
class UnquantizedLinearMethod(LinearMethodBase):
    ...
    def load_weights_vanilla(self, module: torch.nn.Module, weights: List[Dict],
                             device: torch.device):
        load_weights_vanilla_helper(module, weights, device)

    def load_weights_fused_qkv_linear(self, module: torch.nn.Module,
                                      weights: List[Dict],
                                      device: torch.device):
        q_weight, k_weight, v_weight = load_weights_fused_qkv_helper(
            module, weights, device)
...
class FP8QDQLinearMethod(LinearMethodBase):
    ...
    def load_weight_scales_fp8_qdq(self, weights: List[Dict]):
    ...
    def load_weights_vanilla(self, module: torch.nn.Module, weights: List[Dict],
                             device: torch.device):
        load_weights_vanilla_helper(module, weights, device)
        input_scale, weight_scale = self.load_weight_scales_fp8_qdq(weights)
...
class NVFP4LinearMethod(LinearMethodBase):
    ...
    def load_weight_scales_nvfp4(self,
                                 weights: List[Dict],
                                 tp_size: int = 1,
                                 tp_rank: int = 0,
    ...
    def load_weights_vanilla(self, module: torch.nn.Module, weights: List[Dict],
        ...
        input_scale, weight_scale, alpha = self.load_weight_scales_nvfp4(
            weights,
    ...
    def load_weights_fused_qkv_linear(self, module: torch.nn.Module,
        ...
        input_scale, weight_scales, alpha = self.load_weight_scales_nvfp4(
            weights,
            tp_size=module.tp_size,
            tp_rank=module.tp_rank,
            tp_mode=module.tp_mode)
