...
from typing import NamedTuple, List, Dict, Optional, override
...
class FusedMoEMethodBase:
    ...
    def create_weights(self, module: torch.nn.Module, weight_dtype: torch.dtype, w3_w1_weight_shape: tuple[int, int, int], w2_weight_shape: tuple[int, int, int]):
    ...
    def load_weights(self, module: torch.nn.Module, weights: List[Dict], initial_local_expert_ids: List[int],
        ...
        for local_slot_id, expert_id in enumerate(
            ...
            thread = threading.Thread(target=self.load_expert_w3_w1_weight,
    ...
    def load_expert_w3_w1_weight(self, module: torch.nn.Module, w1_weight: torch.Tensor, w3_weight: torch.Tensor, dst_w3_w1_weight: torch.Tensor):
    ...
    def load_expert_w2_weight(self, module: torch.nn.Module, w2_weight: torch.Tensor, dst_w2_weight: torch.Tensor):
        w2_weight_shard = load_weight_shard(w2_weight, module.tp_size,
                                            module.tp_rank,
                                            TensorParallelMode.ROW)
        dst_w2_weight.copy_(w2_weight_shard.view(
            dst_w2_weight.dtype),
                                non_blocking=True)
...
class UnquantizedFusedMoEMethod(FusedMoEMethodBase):
    ...
    def create_weights(self, module: torch.nn.Module):
        ...
        super().create_weights(module, weight_dtype, w3_w1_weight_shape, w2_weight_shape)
...
class FP8QDQFusedMoEMethod(FusedMoEMethodBase):
    ...
    def create_weights(self, module: torch.nn.Module):
        ...
        fc31_input_dequant = nn.Parameter(torch.tensor(
            1., dtype=torch.float32),
                                            requires_grad=False)
        module.register_parameter("fc31_input_dequant",
                                fc31_input_dequant)
    ...
    def load_quant_scales(self, module: torch.nn.Module, weights: Dict):
        ...
        def _load_expert_w3_w1_weight_scale(
        ...
        def _load_expert_fc2_input_scale_fp8_qdq(
        ...
        for expert_id in range(module.num_experts):
            ...
            _load_expert_w3_w1_weight_scale(
                w1_input_scale, w3_input_scale, tmp_fc31_input_scale[expert_id])

            _load_expert_fc2_input_scale_fp8_qdq(w2_input_scale,
        ...
        def _load_expert_w3_w1_weight_scale_fp8_qdq(
        ...
        def _requantize_expert_w3_w1_weight_fp8_qdq(
                w1_weight_scale, w3_weight_scale,
        ...
        def _load_expert_w2_weight_scale_fp8(w2_weight_scale,
        ...
        for local_slot_id, expert_id in enumerate(
            ...
            _load_expert_w3_w1_weight_scale_fp8_qdq(
                w1_weight_scale, w3_weight_scale,
                tmp_w3_w1_weight_scale[expert_idx])

            _requantize_expert_w3_w1_weight_fp8_qdq(
                w1_weight_scale, w3_weight_scale,
                module.w3_w1_weight.data[expert_idx])

            _load_expert_w2_weight_scale_fp8(w2_weight_scale,
    ...
    def load_quant_scales(self, module: torch.nn.Module, weights: Dict):
        ...
        all_w3_scales = [
            load_weight_shard(weights[f"{expert_id}.w3.weight_scale_inv"],
                              ...
                              TensorParallelMode.COLUMN, device=device)
        ...
        all_w2_scales = [
            load_weight_shard(weights[f"{expert_id}.w2.weight_scale_inv"],
                              module.tp_size, module.tp_rank,
                              TensorParallelMode.ROW, device=device)
            for expert_id in module.initial_local_expert_ids
        ]
...
class WInt4AFP8FusedMoEMethod(FusedMoEMethodBase):
    ...
    def create_weights(self, module: torch.nn.Module):
        ...
        fc31_act_scale = nn.Parameter(torch.empty(
            module.expert_size_per_partition, 1, dtype=module.dtype),
                                        requires_grad=False)
        ...
        fc31_alpha = nn.Parameter(torch.empty(
            module.expert_size_per_partition, 1, dtype=torch.float32),
                                    requires_grad=False)
    ...
    def load_quant_scales(self, module: torch.nn.Module, weights: Dict):
        ...
        module.fc31_act_scale.data.copy_(1 / all_w3_w1_input_scales)
        module.fc31_alpha.data.copy_(all_w3_w1_input_scales.float())
        ...
        module.fc31_weight_scale.data.copy_(w3_w1_scales_interleaved.contiguous())
        ...
        module.fc2_act_scale.data.copy_(1 / all_w2_input_scales)
        module.fc2_alpha.data.copy_(all_w2_input_scales.float())
        ...
        w2_scales_interleaved = w2_scales.reshape(
            w2_s_shape[0], w2_s_shape[1], (w2_s_shape[2] // module.interleave[1]),
            module.interleave[1])
...
class NVFP4FusedMoEMethod(FusedMoEMethodBase):
    ...
    @override
    def create_weights(self, module: torch.nn.Module, weight_dtype, weight_vec_size, block_scales_dtype, block_scales_vec_size):
        ...
        w3_w1_weight_scale = nn.Parameter(
            ...
            requires_grad=False)
        module.register_parameter("w3_w1_weight_scale",
                                w3_w1_weight_scale)
    ...
    @abstractmethod
    def load_expert_w3_w1_weight_scale(self, module: torch.nn.Module, w1_weight_scale: torch.Tensor, w3_weight_scale: torch.Tensor, dst_w3_w1_weight_scale: torch.Tensor):
        pass

    @abstractmethod
    def load_expert_w2_weight_scale(self, module: torch.nn.Module, w2_weight_scale: torch.Tensor, dst_w2_weight_scale: torch.Tensor):
        pass

    def load_quant_scales(self, module: torch.nn.Module, weights: Dict, block_scales_dtype: torch.dtype, load_expert_w3_w1_weight_scale_nvfp4: Callable, load_expert_w2_weight_scale_nvfp4: Callable):
        # Step1: Load input scales.
        tmp_fc31_input_scale = torch.empty(module.num_experts,
                                           dtype=torch.float32)
        tmp_fc2_input_scale = torch.empty(self.num_experts, dtype=torch.float32)

        def _load_expert_fc31_input_scale_nvfp4(
                w1_input_scale, w3_input_scale,
                dst_fc31_input_scale: torch.Tensor):
        ...
        def _load_expert_fc2_input_scale_nvfp4(
                w2_input_scale, dst_fc2_input_scale: torch.Tensor):
            dst_fc2_input_scale.copy_(w2_input_scale[...].reshape([]))
        ...
        for expert_id in range(module.num_experts):
            ...
            _load_expert_fc31_input_scale_nvfp4(w1_input_scale, w3_input_scale,
                                               tmp_fc31_input_scale[expert_id])
            _load_expert_fc2_input_scale_nvfp4(w2_input_scale,
                                              tmp_fc2_input_scale[expert_id])
        ...
        def _load_expert_fc31_alpha_nvfp4(w1_weight_scale_2, w3_weight_scale_2,
        ...
        def _load_expert_fc2_alpha_nvfp4(w2_weight_scale_2,
        ...
        for local_slot_id, expert_id in enumerate(
            ...
            _load_expert_fc31_alpha_nvfp4(w1_weight_scale_2, w3_weight_scale_2,
                                         self.fc31_input_scale.data,
                                         self.fc31_alpha.data[expert_idx])
            _load_expert_fc2_alpha_nvfp4(w2_weight_scale_2,
                                        self.fc2_input_scale.data,
                                        self.fc2_alpha.data[expert_idx])


class NVFP4CutlassFusedMoEMethod(FusedMoEMethodBase):
    ...
    def create_weights(self, module: torch.nn.Module):
        ...
        super().create_weights(module, weight_dtype, weight_vec_size, block_scales_dtype, block_scales_vec_size)
...
class NVFP4TRTLLMGenFusedMoEMethod(FusedMoEMethodBase):
    ...
    def load_expert_w3_w1_weight(self, module: torch.nn.Module, w1_weight: torch.Tensor, w3_weight: torch.Tensor, dst_w3_w1_weight: torch.Tensor):
        ...
        dst_w3_weight, dst_w1_weight = dst_w3_w1_weight.split(
                    self.intermediate_size_per_partition, dim=0)
